...
class Participant:
    def __init__(
        ...
        if query_strategy is not None:
            ...
            from .strategies import FullScanStrategy
            self.query_strategy = FullScanStrategy()
        if cache_strategy is not None:
            self.cache_strategy = cache_strategy
        else:
            from .strategies import InfiniteCacheStrategy
            self.cache_strategy = InfiniteCacheStrategy()
    ...
    def receive_event(self, case_id: str, log_index: int, activity: str):
        self.local_events[(case_id, log_index)] = activity
        self.network.record_local_event(self.id, case_id)

    def add_entrypoint(self, entrypoint: Entrypoint):
        self.entrypoints.append(entrypoint)
        self.network.activity_index.setdefault(entrypoint.activity, []).append(self)
    ...
    def process_event(self, event: Event, log_index: int) -> Alignment:
        ...
        for i, p in enumerate(ranked):
            if self.query_strategy.should_skip_participant(p, best_cost):
                continue

            cost, entrypoint = p.compute_best_cost(log_index, event.case_id)
            self.query_strategy.notify_computed(p.id, event.case_id, cost)
            ...
            if self.query_strategy.should_stop_early(best_cost, ranked[i + 1:]):
                break
